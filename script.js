...
function resetProjectsToDefault() {
  ...
  const defaultItem = document.querySelector(".project-item");
  if (!defaultItem) return;

  const defaultImages = getImagesFromItem(defaultItem);

  if (defaultImages.length) {
    currentProjectImages = defaultImages;
    currentProjectIndex = 0;
    buildViewerDots(defaultImages.length);
    updateViewer();
  } else if (defaultItem.dataset.preview && viewerImage) {
    viewerImage.src = defaultItem.dataset.preview;
    if (viewerDots) {
      viewerDots.innerHTML = "";
    }
  }
